 ...
 * @brief 
 * 	This file contains the csp class and its function declarations. The class is used in solving the constraint 
 *  satisfication probelm. Forward checking, minimum remaining value, degree heuristic, and arc consistency are 
 *  implemented to help solve csp.
 * @date 2022-10-31
 * 
...
class CSP 
{
	...
	private:
		...
		void LoadState(std::map<Variable*, std::set<typename CSP<T>::Variable::Value> >& saved) const;
		...
		//except the current
		std::map<Variable*, std::set<typename Variable::Value> > SaveState(Variable* x) const;
